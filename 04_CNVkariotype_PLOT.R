...
p1<-lapply(1:length(cldat), function(i) { NM<-names(cldat)[i];
					  ...
					  png(outnm)
					  p<-kp_f(samp_ID=NM,data=DT,reference= ref_CNV, skip= FALSE, plot_type="bar",chrom=CHR, mp="midpoint",y1="ProbXncell", scale=TRUE, colR=NULL);
					  dev.off()
...
p2<- lapply(1:length(cldat), function(i) {NM<-names(cldat)[i];
					  ...
					  outnm<-paste(outD,nm1,sep="");
					  png(outnm)
					  p<-kp_f(samp_ID=NM,data=DT,reference= ref_CNV, skip= FALSE, plot_type="area",chrom=CHR, mp="midpoint",y1="ProbXncell",scale=TRUE, colR=NULL);
					  dev.off()
					  cat(".. plot 1 area karyotype for sample:\n.. ..",NM,"\n");
					  rm(NM);rm(DT);rm(CHR);rm(nm1);rm(outnm);
					  return(p)})
...
p3<-lapply(1:length(cldat), function(i) { NM<-names(cldat)[i];
					  ...
					  outnm<-paste(outD,nm1,sep="");
					  png(outnm)
					  p<-kp_f(samp_ID=NM,data=DT,reference= ref_CNV, skip= FALSE, plot_type="bar",chrom=CHR, mp="midpoint",y1="ProbXpcell", scale=FALSE, colR=NULL);
					  dev.off()
					  cat(".. plot 1 bar karyotype for sample:\n.. ..",NM,"\n");
					  rm(NM);rm(DT);rm(CHR);rm(nm1);rm(outnm);
					  return(p)})
...
p4<- lapply(1:length(cldat), function(i) {NM<-names(cldat)[i];
					  ...
					  outnm<-paste(outD,nm1,sep="");
					  png(outnm)
					  p<-kp_f(samp_ID=NM,data=DT,reference= ref_CNV, skip= FALSE, plot_type="area",chrom=CHR, mp="midpoint",y1="ProbXpcell",scale=FALSE, colR=NULL);
					  dev.off()
					  cat(".. plot 1 area karyotype for sample:\n.. ..",NM,"\n");
					  rm(NM);rm(DT);rm(CHR);rm(nm1);rm(outnm);
					  return(p)})
